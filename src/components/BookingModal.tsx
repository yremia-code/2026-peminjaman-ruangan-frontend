...
const BookingModal = ({ isOpen, onClose, onSubmit, initialData }: BookingModalProps) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (formData.tanggalSelesai <= formData.tanggalPinjam) {
            alert("⚠️ Waktu Selesai tidak boleh lebih awal atau sama dengan Waktu Mulai!");
            return;
        }
        ...
        <div className="modal-overlay">
            <div className="modal-content">
                ...
                <form onSubmit={handleSubmit}>
                    {isLoadingOptions && <p style={{ fontSize: '0.8rem', color: '#666' }}>Memuat data user & ruangan...</p>}
                    ...
                    <div className="form-group">
                        ...
                        <select
                            ...
                            onChange={(e) => setFormData({ ...formData, ruanganId: Number(e.target.value) })}
                            required
                        >
                            <option value={0}>-- Pilih Ruangan --</option>
                            {rooms.map(r => (
                                <option key={r.id} value={r.id}>{r.nama} - {r.gedung}</option>
                            ))}
                        </select>
                    </div>
                    ...
                    <div className="form-group">
                        ...
                        {formData.tanggalPinjam && (
                            <small style={{ color: '#666', fontSize: '0.75rem' }}>
                                *Harus setelah {new Date(formData.tanggalPinjam).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                            </small>
                        )}
                    </div>
